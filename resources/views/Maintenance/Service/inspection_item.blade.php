			...
			@foreach($inspect_item as $inspectItem)
				@if($inspectItem->inspectItemIsActive==1)
					<tr>
						...
						<td>
							<button class="ui green basic button" name="edit{{ $inspectItem->inspectItemId }}" onclick="modal(this.name)"><i class="write icon"></i>Edit</button>
							<button class="ui red basic button" name="del{{ $inspectItem->inspectItemId }}" onclick="modal(this.name)"><i class="trash icon"></i>Delete</button>
						</td>
						<!--Modal for Edit-->
